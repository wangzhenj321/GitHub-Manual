# GitHub-Manual
Basic manual about how to use GitHub and Git

## Basic

1. [Detached HEAD](blog/Detached-HEAD.md)
2. [From Issue to Pull Request to Merge](blog/From-Issue-to-Pull-Request-to-Merge.md)
3. [Objects and Index](blog/objects-and-index.md)

## Commands

1. [git add](blog/git-add.md)
2. [git branch](blog/git-branch.md)
3. [git checkout](blog/git-checkout.md)
4. [git cherry-pick](blog/git-cherry‐pick.md)
5. [git clean](blog/git-clean.md)
6. [git clone](blog/git-clone.md)
7. [git commit](blog/git-commit.md)
8. [git config](blog/git-config.md)
9. [git diff](blog/git-diff.md)
10. [git fetch](blog/git-fetch.md)
11. [git gc](blog/git-gc.md)
12. [git help](blog/git-help.md)
13. [git init](blog/git-init.md)
14. [git log](blog/git-log.md)
15. [git merge/rebase/squash](blog/git-merge-rebase-squash.md)
16. [git merge](blog/git-merge.md)
17. [git pull](blog/git-pull.md)
18. [git push](blog/git-push.md)
19. [git rebase -i](blog/git-rebase-‐i.md)
20. [git remote](blog/git-remote.md)
21. [git reset](blog/git-reset.md)
22. [git revert](blog/git-revert.md)
23. [git rm](blog/git-rm.md)
24. [git show](blog/git-show.md)
25. [git stash](blog/git-stash.md)
26. [git status](blog/git-status.md)
27. [git submodule and .gitmodules](blog/git-submodule-and-gitmodules.md)
28. [git tag](blog/git-tag.md)

## Guides

1. [gitrevisions](blog/gitrevisions.md)

## Q & A

1. [Cache your GitHub password](blog/Cache-your-GitHub-password.md)
2. [Clean up unused git branches](blog/Clean-up-unused-git-branches.md)
3. [Rename a directory](blog/Rename-a-directory.md)
4. [Update a forked repo](blog/Update-a-forked-repo.md)
5. [Upload local repo to GitHub](blog/Upload-local-repo-to-GitHub.md)
6. [Use SSH over HTTPS port(443)](blog/Use-SSH-over-HTTPS-port-(443).md)
7. [gitignore is not working](blog/gitignore-is-not-working.md)
8. [Git Merge conflicts resolution made easy](blog/git-merge-conflicts-resolution-made-easy.md)

## Tools

1. [Markdown](blog/markdown.md)

## References

1. [git doc](https://git-scm.com/doc)
2. [How to Use Git and GitHub on Udacity](https://www.udacity.com/course/how-to-use-git-and-github--ud775)
