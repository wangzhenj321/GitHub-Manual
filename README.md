# GitHub-Manual
Basic manual about how to use GitHub and Git

## Basic

1. [Detached HEAD](blog/Detached-HEAD.md)
1. [From Issue to Pull Request to Merge](blog/From-Issue-to-Pull-Request-to-Merge.md)
1. [Objects and Index](blog/objects-and-index.md)

## Commands

1. [git add](blog/git-add.md)
1. [git am](blog/git-am.md)
1. [git apply](blog/git-apply.md)
1. [git branch](blog/git-branch.md)
1. [git checkout](blog/git-checkout.md)
1. [git cherry-pick](blog/git-cherry‐pick.md)
1. [git clean](blog/git-clean.md)
1. [git clone](blog/git-clone.md)
1. [git commit](blog/git-commit.md)
1. [git config](blog/git-config.md)
1. [git diff](blog/git-diff.md)
1. [git fetch](blog/git-fetch.md)
1. [git format-patch](blog/git-format-patch.md)
1. [git gc](blog/git-gc.md)
1. [git help](blog/git-help.md)
1. [git init](blog/git-init.md)
1. [git log](blog/git-log.md)
1. [git merge/rebase/squash](blog/git-merge-rebase-squash.md)
1. [git merge](blog/git-merge.md)
1. [git prune](blog/git-prune.md)
1. [git pull](blog/git-pull.md)
1. [git push](blog/git-push.md)
1. [git rebase -i](blog/git-rebase-‐i.md)
1. [git remote](blog/git-remote.md)
1. [git reset](blog/git-reset.md)
1. [git revert](blog/git-revert.md)
1. [git rm](blog/git-rm.md)
1. [git show](blog/git-show.md)
1. [git stash](blog/git-stash.md)
1. [git status](blog/git-status.md)
1. [git submodule and .gitmodules](blog/git-submodule-and-gitmodules.md)
1. [git tag](blog/git-tag.md)

## Guides

1. [gitrevisions](blog/gitrevisions.md)

## Q & A

1. [Cache your GitHub password](blog/Cache-your-GitHub-password.md)
1. [Clean up unused git branches](blog/Clean-up-unused-git-branches.md)
1. [Move files from one repository to another](blog/move-files-from-one-repository-to-another.md)
1. [Rename a directory](blog/Rename-a-directory.md)
1. [Update a forked repo](blog/Update-a-forked-repo.md)
1. [Upload local repo to GitHub](blog/Upload-local-repo-to-GitHub.md)
1. [Use SSH over HTTPS port(443)](blog/Use-SSH-over-HTTPS-port-(443).md)
1. [gitignore is not working](blog/gitignore-is-not-working.md)
1. [Git Merge conflicts resolution made easy](blog/git-merge-conflicts-resolution-made-easy.md)
1. [Exclude a directory from git diff](blog/Exclude-a-directory-from-git-diff.md)

## Tools

1. [Markdown](blog/markdown.md)

## References

1. [git doc](https://git-scm.com/doc)
1. [How to Use Git and GitHub on Udacity](https://www.udacity.com/course/how-to-use-git-and-github--ud775)
